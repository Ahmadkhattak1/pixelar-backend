...
// Animation presets based on retro-diffusion/rd-animation model
export interface AnimationPreset {
    id: string;
    name: string;
    description: string;
    style: string; // rd-animation style parameter
    width: number;
    height: number;
    frameCount: number;
    recommended: boolean;
...
export interface SpritesheetGenerationParams {
    characterImageUrl: string; // URL or base64 of the source character
    animationPresetId: string;
    customPrompt?: string; // Additional description for the animation
    customWidth?: number; // Override preset width
    customHeight?: number; // Override preset height
}

export interface SpritesheetGenerationResult {
    success: boolean;
    spritesheetUrl?: string; // URL to the generated spritesheet PNG
    gifUrl?: string; // URL to the animated GIF (if requested)
    frameCount: number;
    frameWidth: number;
    frameHeight: number;
...
async function callReplicateAnimation(
    apiToken: string,
    input: any
): Promise<any> {
    ...
    let prediction = await startResponse.json() as ReplicatePrediction;
...
export async function generateSpritesheet(
    params: SpritesheetGenerationParams,
    apiToken: string
): Promise<SpritesheetGenerationResult> {
    try {
        // Find the preset
        const preset = ANIMATION_PRESETS.find(p => p.id === params.animationPresetId);
        if (!preset) {
 ...
 */
export function getAnimationPresets(): AnimationPreset[] {
